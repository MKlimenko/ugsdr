...
namespace ugsdr {
	...
	protected:
		...
		static void Process(std::vector<std::complex<UnderlyingType>>& src_dst, bool is_inverse = false) {
			...
			Ipp32s init_size = 0;
			Ipp32s work_size = 0;
			IppDftFunctions<UnderlyingType>::GetSize(static_cast<int>(src_dst.size()), IPP_FFT_DIV_INV_BY_N, ippAlgHintNone, &spec_size, &init_size, &work_size);
			...
			IppDftFunctions<UnderlyingType>::GetInit(static_cast<int>(src_dst.size()), IPP_FFT_DIV_INV_BY_N, ippAlgHintNone, reinterpret_cast<DftSpecType*>(spec_ptr), init_buf);
			dft_routine(reinterpret_cast<IppType*>(src_dst.data()), reinterpret_cast<IppType*>(src_dst.data()), reinterpret_cast<DftSpecType*>(spec_ptr), work_buf);
		}
